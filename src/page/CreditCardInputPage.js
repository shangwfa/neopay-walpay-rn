...
class CreditCardInputPage extends BasePage {
    ...
    render() {
        ...
        const nameData = {'key': 'CVV2', 'placeholder': '信用卡背面签名栏末三位数字', isLine: true}
        const idCardNameData = {'key': '有效期', 'placeholder': '请选择信用卡有效期', isLine: true}
        ...
        return (
            <View style={styles.container}>
                <Header navigation={this.props.navigation} title='信用卡补充信息'/>
                <View style={{height: 10}}/>
                <CommonInput data={nameData} editable={false} onChangeText={(text) => this.setState({name: text})}/>
                <CommonInput data={idCardNameData} editable={false} onChangeText={(text) => this.setState({idCardNo: text})}/>
