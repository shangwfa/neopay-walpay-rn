...
import com.xgjk.common.lib.view.rollViewPager.HintView;
...
public abstract class ShapeHintView extends LinearLayout implements HintView {
    ...
    public void setmDotsMargin(int left, int top, int right, int bottom) {
        Logger.d("childCount:" + getChildCount());
        for (int i = 0; i < getChildCount(); i++) {
            final View view = getChildAt(i);
            LayoutParams dotlp = (LayoutParams) view.getLayoutParams();
            dotlp.setMargins(left, top, right, bottom);
            view.setLayoutParams(dotlp);
        }
    }
